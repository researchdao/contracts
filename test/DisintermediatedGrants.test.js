...
describe('DisintermediatedGrants', function () {
    ...
    beforeEach(async function () {
        this.grants = await this.DisintermediatedGrants.connect(this.owner).deploy(this.multisig.address, DONATION_GRACE_PERIOD);
    ...
    describe('donors', function () {
        it('cannot be whitelisted by non-owner', async function () {
            ...
            await expect(this.grants.connect(this.eve).whitelistDonor(this.eve.address)).to.be.revertedWith('Ownable: caller is not the owner');
        });
    ...
    describe('ERC20 donations', function () {
        it('cannot be made by non-whitelisted donors', async function () {
            await expect(this.grants.connect(this.eve).donate(this.testERC20.address, ERC20_TOKEN_AMOUNT)).to.be.revertedWith('caller is not whitelisted donor');
        });
        it('fail if donation amount exceeds donor balance', async function () {
            ...
            await expect(this.grants.connect(this.alice).donate(this.testERC20.address, donorBalance.add(1))).to.be.revertedWith('ERC20: transfer amount exceeds balance');
        });
        it('fail if donation amount exceeds donor allowance', async function () {
            await whitelistDonor(this.grants, this.alice.address);
            const donorBalance = await this.testERC20.balanceOf(this.alice.address);
            await this.testERC20.connect(this.alice).mint(1);
            await expect(this.grants.connect(this.alice).donate(this.testERC20.address, donorBalance.add(1))).to.be.revertedWith('ERC20: transfer amount exceeds allowance');
        });
    ...
    describe('native donations', function () {
        it('cannot be made by non-whitelisted donors', async function () {
            await expect(this.grants.connect(this.eve).donateNative({ value: ETH_AMOUNT })).to.be.revertedWith('caller is not whitelisted donor');
        });
        it('can be made by whitelisted donors', async function () {
            await whitelistDonor(this.grants, this.alice.address);
            const donationCount = await this.grants.donationCount();
    ...
    describe('donation withdrawal', function () {
        ...
        it('fails for ERC20 donations if caller is not the donor', async function () {
            ...
            await expect(this.grants.connect(this.eve).withdrawDonation(donationId)).to.be.revertedWith('caller is not donor');
        });
        ...
        it('fails for ERC20 donations that have been fully disbursed', async function () {
            ...
            await expect(this.grants.connect(this.alice).withdrawDonation(donationId)).to.be.revertedWith('donation has been fully disbursed');
        });
        ...
        it('fails if ERC20 donation amount exceeds contract balance', async function () {
            ...
            await expect(this.grants.connect(this.alice).withdrawDonation(donationId)).to.be.revertedWith('ERC20: transfer amount exceeds balance');
        });
        it('fails if native donation amount exceeds contract balance', async function () {
            ...
            await expect(this.grants.connect(this.alice).withdrawDonation(donationId)).to.be.revertedWith('failed to withdraw donation');
        });
        it('withdraws ERC20 funds to donor', async function () {
            ...
            await expect(tx).to.emit(this.testERC20, 'Transfer').withArgs(this.grants.address, this.alice.address, withdrawalAmount);
        ...
        it('withdraws native funds to donor', async function () {
            ...
            expect(donation.withdrawn).to.equal(true);
            await expect(tx).to.emit(this.grants, 'WithdrawDonation').withArgs(donation);
            expect(await ethers.provider.getBalance(this.alice.address)).to.equal(donorBalance.sub(receipt.gasUsed.mul(receipt.effectiveGasPrice)).add(withdrawalAmount));
        });
    });
    describe('grant proposals', function () {
        it('cannot be created by non-owner', async function () {
            ...
            await expect(this.grants.connect(this.eve).proposeGrant(donationId, this.eve.address, ETH_AMOUNT)).to.be.revertedWith('Ownable: caller is not the owner');
        });
        it('fail if donation does not exist', async function () {
            await expect(this.grants.proposeGrant(404, this.eve.address, ETH_AMOUNT.mul(2))).to.be.revertedWith('donation cannot cover full grant amount');
        });
        it('fail if donation cannot cover full grant amount', async function () {
            ...
            await expect(this.grants.proposeGrant(donationId, this.eve.address, ETH_AMOUNT.mul(2))).to.be.revertedWith('donation cannot cover full grant amount');
        });
    ...
    describe('single grant endorsements', function () {
        it('cannot be created by non-multisig accounts', async function () {
            const donationId = await setDonation(this.grants, this.defaultNativeDonation);
            await expect(this.grants.connect(this.eve).endorseGrant(donationId)).to.be.revertedWith('caller is not the multisig');
        });
    ...
    describe('multiple grant endorsements', function () {
        it('cannot be created by non-multisig accounts', async function () {
            const [donationAId, donationBId] = [await setDonation(this.grants, this.defaultERC20Donation), await setDonation(this.grants, this.defaultNativeDonation)];;
            await expect(this.grants.connect(this.eve).endorseGrants([donationAId, donationBId])).to.be.revertedWith('caller is not the multisig');
        });
        it('can be created by the multisig', async function () {
            const [donationAId, donationBId] = [await setDonation(this.grants, this.defaultERC20Donation), await setDonation(this.grants, this.defaultNativeDonation)];;
            const grantCount = await this.grants.grantCount();
    ...
    describe('grant disbursal', function () {
        ...
        it('fails if ERC20 grant amount exceeds contract balance', async function () {
            ...
            await expect(this.grants.disburseGrant(grantId)).to.be.revertedWith('ERC20: transfer amount exceeds balance');
        });
        it('transfers ERC20 grant amount to recipient', async function () {
            ...
            await expect(tx).to.emit(this.testERC20, 'Transfer').withArgs(this.grants.address, this.bob.address, grant.amount);
